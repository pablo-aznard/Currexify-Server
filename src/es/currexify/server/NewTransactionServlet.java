...
public class NewTransactionServlet extends HttpServlet {
	...
	String[] times = { "Instantánea -- 1.5%", "3 días -- 1.3%",
			"7 días -- 1%" };
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		int days = 0;
		double charge = 0.015;
		if (time.equals(times[1])) {
			days = 3;
			charge = 0.013;
		}
		if (time.equals(times[2])) {
			days = 7;
			charge = 0.01;
		}
		...
		TransactionModel tm = new TransactionModel(usuario.getCardN(), from,
				to, Double.parseDouble(amount), cal.getTime(), charge);
		udao.addTransactionToUser(em, tm,
				udao.readUserByEmail(em, usuario.getEmail()));

		cal.add(Calendar.MINUTE, days * -1);
